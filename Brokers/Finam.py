...
from FinamPy.grpc.orders_service_pb2 import OrdersRequest, OrdersResponse, OrderType, OrderState, OrderStatus, Order as FinamOrder, StopCondition, CancelOrderRequest  # Заявки
...
class Finam(Broker):
    ...
    def get_symbol_by_dataname(self, dataname):
        ...
        if finam_board is None:  # Если режим торгов не найден
        ...
        while seconds_from <= seconds_to:  # Пока нужно получать данные
            ...
            if len(bars_response.bars) > 0:  # Если за период получены бары
                ...
                for bar in bars_response.bars:  # Пробегаемся по всем пришедшим барам
                    ...
                    open_ = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(bar.open.value))  # Конвертируем цены
                    high = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(bar.high.value))  # из цен Финама
                    low = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(bar.low.value))  # в зависимости от
                    close = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(bar.close.value))  # режима торгов
    ...
    def get_last_price(self, symbol):
        ...
        return None if quote_response is None else self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(quote_response.quote.last.value))  # Последняя цена сделки
    ...
    def get_value(self):
        ...
        return round(float(account.equity.value), 2)  # Стоимость портфеля
    ...
    def get_positions(self):
        ...
        for position in account.positions:  # Пробегаемся по всем позициям
            ...
            if symbol is None:  # Если тикер не найден
            ...
            self.positions.append(Position(  # Добавляем текущую позицию в список
                ...
                self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(position.average_price.value)) if position.average_price.value != '' else 0,  # Средняя цена входа в рублях. Для фьючерсов не задается
                self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(position.current_price.value))))  # Последняя цена в рублях
    ...
    def get_orders(self):
        ...
        for order in orders.orders:  # Пробегаемся по всем заявкам
            ...
            if exec_type in (Order.Limit, Order.StopLimit):
                price = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(order.order.limit_price.value))  # Лимитная цена
            if exec_type in (Order.Stop, Order.StopLimit):
                stop_price = self.provider.finam_price_to_price(symbol.symbol, symbol.broker_info['mic'], float(order.order.stop_price.value))  # Цена срабатывания
    ...
    def new_order(self, order):
        ...
        if symbol is None:  # Если тикер не найден
            return False  # то операция завершилась с ошибкой. Выходим, дальше не продолжаем
        ...
        limit_price = Decimal(value=str(round(self.provider.price_to_finam_price(symbol.symbol, symbol.broker_info['mic'], order.price), symbol.decimals)))  # Лимитная цена Финама
        stop_price = Decimal(value=str(round(self.provider.price_to_finam_price(symbol.symbol, symbol.broker_info['mic'], order.stop_price), symbol.decimals)))  # Стоп цена Финама
    ...
    def subscribe_transactions(self):
        self.provider.on_order.subscribe(self._on_order)  # Подписываемся на заявки
        self.provider.on_trade.subscribe(self._on_trade)  # Подписываемся на сделки
        Thread(target=self.provider.subscribe_orders_thread, args=(self.account_id,), name='SubscriptionOrdersThread').start()  # Создаем и запускаем поток обработки своих заявок по счету
        Thread(target=self.provider.subscribe_trades_thread, args=(self.account_id,), name='SubscriptionTradesThread').start()  # Создаем и запускаем поток обработки своих сделок по счету

    def unsubscribe_transactions(self):
        self.provider.on_order.unsubscribe(self._on_order)  # Сбрасываем обработчик заявок
        self.provider.on_trade.unsubscribe(self._on_trade)  # Сбрасываем обработчик сделок
    ...
    def _on_new_bar(self, bars: SubscribeBarsResponse, timeframe: TimeFrame.ValueType):
        ...
        if (symbol, time_frame) not in self.history_subscriptions:  # Если была отписка от тикера
            return  # Выходим, дальше не продолжаем
        last_bar = self.last_bars.get((symbol.dataname, time_frame))  # Последний бар. Он может быть не завершен
    ...
    def _on_order(self, order: OrderState):
        ...
        # Тип заявки
        order_type_map = {
            OrderType.ORDER_TYPE_MARKET: Order.Market,  # Рыночная заявка
            OrderType.ORDER_TYPE_LIMIT: Order.Limit,  # Лимитная заявка
            OrderType.ORDER_TYPE_STOP: Order.Stop,  # Стоп заявка
            OrderType.ORDER_TYPE_STOP_LIMIT: Order.StopLimit,  # Стоп лимитная заявка
        }
        if order.order.type not in order_type_map:  # Если типа заявки нет в справочнике
            raise NotImplementedError(f'Тип заявки {order.order.type} не поддерживается')  # то с ними (мульти лег) не работаем
        order_type = order_type_map[order.order.type]  # то возвращаем тип заявки FinLabPy

        # Статус заявки
        if order.status == OrderStatus.ORDER_STATUS_FORWARDING:  # Исполнение стоп заявки, создание рыночной/лимитной заявки
            # client_order_id = order.order.client_order_id  # По этому номеру можно связать стоп и рыночную/лимитную заявки
            return  # Исполнение стоп заявки получим далее со статусом OrderStatus.ORDER_STATUS_EXECUTED. Выходим, дальше не продолжаем
        order_status_map = {
            OrderStatus.ORDER_STATUS_NEW: Order.Created,  # Новая заявка
            OrderStatus.ORDER_STATUS_REJECTED: Order.Rejected,  # Отклонена
            OrderStatus.ORDER_STATUS_REJECTED_BY_EXCHANGE: Order.Rejected,  # Отклонена биржей
            OrderStatus.ORDER_STATUS_FAILED: Order.Rejected,  # Ошибка
            OrderStatus.ORDER_STATUS_DENIED_BY_BROKER: Order.Rejected,  # Отклонена брокером
            OrderStatus.ORDER_STATUS_WAIT: Order.Accepted,  # Ожидает исполнения (лимитная заявка)
            OrderStatus.ORDER_STATUS_WATCHING: Order.Accepted,  # Наблюдение (стоп заявка)
            OrderStatus.ORDER_STATUS_CANCELED: Order.Canceled,  # Отменена
            OrderStatus.ORDER_STATUS_EXPIRED: Order.Expired, # Истекла
            OrderStatus.ORDER_STATUS_EXECUTED: Order.Completed,  # Исполнена (стоп заявка)
            OrderStatus.ORDER_STATUS_PARTIALLY_FILLED: Order.Partial, # Частично исполнена
            OrderStatus.ORDER_STATUS_FILLED: Order.Completed,  # Исполнена
        }
        if order.status not in order_status_map:  # Если статуса заявки нет в справочнике
            raise NotImplementedError(f'Статус заявки {order.status} не поддерживается')  # то с ними не работаем
        order_status = order_status_map[order.status]  # то возвращаем статус заявки FinLabPy
